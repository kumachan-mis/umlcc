...
Vector* gen_logical_or_expr_immcode(Immcgen* immcgen) {
    ...
    snd_src = new_imm_immcope(fst_src->suffix, 0);
    ...
    fst_src = new_imm_immcope(dst->suffix, 0);
    ...
    fst_src = new_imm_immcope(dst->suffix, 1);
    vector_push(codes, new_inst_immc(IMMC_INST_LOAD, dst, fst_src, NULL));
...
Vector* gen_pointer_additive_expr_immcode(Immcgen* immcgen) {
    ...
    switch (srt->type) {
        case SRT_PADD_EXPR: {
            ...
            ImmcOpe* mul_snd_src = new_imm_immcope(snd_src->suffix, mul_amount);
        ...
        case SRT_PDIFF_EXPR: {
            ...
            ImmcOpe* div_snd_src = new_imm_immcope(dst->suffix, div_amount);
...
Vector* gen_call_expr_immcode(Immcgen* immcgen) {
    ...
    ImmcOpe* prep_src = new_imm_immcope(IMMC_SUFFIX_LONG, num_args);
    ...
    ImmcOpe* snd_src = new_imm_immcope(IMMC_SUFFIX_LONG, num_args);
    ImmcOpe* dst = create_dest_reg_immcope(immcgen);

    vector_push(codes, new_inst_immc(IMMC_INST_CALL, dst, fst_src, snd_src));

    ImmcOpe* clean_src = new_imm_immcope(IMMC_SUFFIX_LONG, num_args);
...
Vector* gen_not_expr_immcode(Immcgen* immcgen) {
    ...
    switch (srt->type) {
        case SRT_LNOT_EXPR: {
            ImmcOpe* zero = new_imm_immcope(src->suffix, 0);
...
Vector* gen_primary_expr_immcode(Immcgen* immcgen) {
    ...
    switch (srt->type) {
        ...
        case SRT_INT_EXPR:
        case SRT_CHAR_EXPR: {
            ImmcSuffix suffix = immcsuffix_get(dtype_size(srt->dtype));
            src = new_imm_immcope(suffix, srt->value_int);
            break;
        }
        default:
            fprintf(stderr, "Error: unexpected srt type %d\n", srt->type);
