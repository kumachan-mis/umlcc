...
Vector* gen_function_definition_immcode(Immcgen* immcgen) {
    ...
    IntegerLiteral* iliteral = new_signed_iliteral(INTEGER_INT, immcgen->symbol_table->memory_nbytes);
    ImmcOpe* memory_nbytes = new_int_immcope(IMMC_SUFFIX_QUAD, iliteral);
    vector_push(codes, new_inst_immc(IMMC_INST_ENTER, NULL, memory_nbytes, NULL));
