...
Vector* gen_add_x64code(X64gen* x64gen) {
    ...
    switch (snd_src->type) {
        case IMMC_OPERAND_IMM: {
            suffix = x64suffix_get(immcsuffix_tosize(fst_src->suffix));
        ...
        case IMMC_OPERAND_REG: {
            ...
            X64Suffix snd_src_suffix = x64suffix_get(immcsuffix_tosize(snd_src->suffix));
...
Vector* gen_divisional_common_x64code(X64gen* x64gen, int result_reg_id) {
    ...
    switch (snd_src->type) {
        case IMMC_OPERAND_IMM: {
            ...
            X64Ope* src = new_imm_x64ope(snd_src->imm_value);
