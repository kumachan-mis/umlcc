...
X64* x64_copy(X64* x64) {
    ...
    if (x64->inst != NULL) {
        copied_x64->inst = x64inst_copy(x64->inst);
    }
    copied_x64->data = NULL;
    if (x64->data != NULL) {
        copied_x64->data = x64data_copy(x64->data);
    }
    copied_x64->label = NULL;
    if (x64->label != NULL) {
        copied_x64->label = x64label_copy(x64->label);
    }
...
void delete_x64(X64* x64) {
    if (x64->inst != NULL) {
        delete_x64inst(x64->inst);
    }
    if (x64->data != NULL) {
        delete_x64data(x64->data);
    }
    if (x64->label != NULL) {
        delete_x64label(x64->label);
    }
    free(x64);
}
