...
int dfunction_equals(DFunction* dfunction, DFunction* other) {
    if (!dtype_equals(dfunction->return_dtype, other->return_dtype)) {
        return 0;
    }
    if (vector_size(dfunction->params) != vector_size(other->params)) {
        return 0;
    }
    ...
    for (int i = 0; i < num_params; i++) {
        ...
        if (!dparam_equals(dfunction_dparam, other_dparam)) {
            return 0;
        }
    }
    return 1;
...
void delete_dfunction(DFunction* dfunction) {
    ...
    if (dfunction->return_dtype != NULL) {
        delete_dtype(dfunction->return_dtype);
    }
    free(dfunction);
}
