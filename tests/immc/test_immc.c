...
void test_immc_tostring_imm_data();
...
CU_Suite* add_test_suite_immc() {
    ...
    CU_add_test(suite, "test_new_inst_immc_1arg", test_new_inst_immc_1arg);
    CU_add_test(suite, "test_new_inst_immc_2arg", test_new_inst_immc_2arg);
    CU_add_test(suite, "test_new_inst_immc_3arg", test_new_inst_immc_3arg);
    CU_add_test(suite, "test_new_imm_data_immc", test_new_imm_data_immc);
    CU_add_test(suite, "test_new_str_data_immc", test_new_str_data_immc);
    CU_add_test(suite, "test_new_label_immc", test_new_label_immc);
    CU_add_test(suite, "test_immc_isjumpinst", test_immc_isjumpinst);
    CU_add_test(suite, "test_immc_tostring_1arg_inst", test_immc_tostring_1arg_inst);
    CU_add_test(suite, "test_immc_tostring_2arg_inst", test_immc_tostring_2arg_inst);
    CU_add_test(suite, "test_immc_tostring_3arg_inst", test_immc_tostring_3arg_inst);
    CU_add_test(suite, "test_immc_tostring_imm_data", test_immc_tostring_imm_data);
    CU_add_test(suite, "test_immc_tostring_str_data", test_immc_tostring_str_data);
    CU_add_test(suite, "test_immc_tostring_label", test_immc_tostring_label);
    return suite;
}
...
void test_new_inst_immc_1arg() {
    ...
    arg = new_imm_immcope(IMMC_SUFFIX_QUAD, 20);
...
void test_new_inst_immc_3arg() {
    ...
    ImmcOpe* snd_src = new_imm_immcope(IMMC_SUFFIX_LONG, 8);
...
void test_new_imm_data_immc() {
    Immc* immc = new_imm_data_immc(IMMC_DATA_WORD, 6);
    ...
    for (int i = 0; i < 2; i++) {
        ...
        CU_ASSERT_EQUAL(immc->data->imm_value, 6);
...
void test_new_str_data_immc() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        CU_ASSERT_EQUAL(immc->data->imm_value, -1);
...
void test_immc_isjumpinst() {
    ...
    snd_src = new_imm_immcope(IMMC_SUFFIX_BYTE, 3);
    ...
    snd_src = new_imm_immcope(IMMC_SUFFIX_NONE, 1);
...
void test_immc_tostring_imm_data() {
    ...
    immc = new_imm_data_immc(IMMC_DATA_BYTE, 8);
    immc_str = immc_tostring(immc);
    CU_ASSERT_STRING_EQUAL(immc_str, "\tbyte\t8\n");
    delete_immc(immc);
    free(immc_str);

    immc = new_imm_data_immc(IMMC_DATA_WORD, 6);
    immc_str = immc_tostring(immc);
    CU_ASSERT_STRING_EQUAL(immc_str, "\tword\t6\n");
    delete_immc(immc);
    free(immc_str);

    immc = new_imm_data_immc(IMMC_DATA_LONG, 19);
    immc_str = immc_tostring(immc);
    CU_ASSERT_STRING_EQUAL(immc_str, "\tlong\t19\n");
    delete_immc(immc);
    free(immc_str);

    immc = new_imm_data_immc(IMMC_DATA_QUAD, 551);
    immc_str = immc_tostring(immc);
    CU_ASSERT_STRING_EQUAL(immc_str, "\tquad\t551\n");
    delete_immc(immc);
    free(immc_str);

    immc = new_imm_data_immc(IMMC_DATA_ZERO, 10);
    immc_str = immc_tostring(immc);
